...
def batch_generator(dataset, train):
	...
	while True:
		if train:
			_, point_x, unused_point_l, img_x = dataset.random_batch()	# point_x: [N, max_seq_len + 1, 5]; point_l: [N]
		...
		target_output = point_x[:, 1:dataset.max_seq_length + 1, :]
		...
		decoder_input = point_x[:, :dataset.max_seq_length, :]

		yield ({'encoder_input': encoder_input, 'decoder_input': decoder_input}, {'output': target_output})

def load_dataset(data_dir, model_params, inference_mode=False, index_list = None, exp_dir = '.'):
	...
	for dataset in datasets:
		...
		for split in splits:
			for im_idx in range(len(data[split])):
				png_path = os.path.join(data_dir, 'png', dataset[:-4], split, str(model_params['img_H']) + 'x' + str(model_params['img_W']), str(im_idx) + '.png')
	...
	print('Dataset combined: {} ({}/{}/{}), avg len {}'.format(len(all_strokes), len(train_strokes), len(valid_strokes), len(test_strokes), int(avg_len)))
	...
	train_set = DataLoader(
		...
		opencv = model_params['opencv'],
		stroke_resize = model_params['stroke_resize']
		)

	normalizing_scale_factor = train_set.calculate_normalizing_scale_factor()
	model_params['normalizing_scale_factor'] = float(normalizing_scale_factor)
	if exp_dir not in '':
		np.save(os.path.join(exp_dir, 'normalizing_scale_factor'), normalizing_scale_factor)
...
class KLWeightScheduler(Callback):
	...
	def on_batch_begin(self, batch, logs=None):
		...
		if self.verbose > 0 and self.count % 20 == 0:
			print('\nBatch %05d: KLWeightScheduler setting KL weight to %s.' % (self.count + 1, new_kl_weight))
		self.count += 1
...
class TensorBoardLR(TensorBoard):
	...
	def on_batch_end(self, batch, logs=None):
		logs.update({'lr': K.eval(self.model.optimizer.lr), 'kl_weight': K.eval(self.kl_weight)})
...
class DataLoader(object):
	...
	def __init__(self,
		...
		# self.png_paths (list)
		self.opencv = opencv
		self.stroke_resize = stroke_resize
		self.preprocess(strokes, png_paths, index_list)
		self.img_h = img_h
		self.img_w = img_w

	def preprocess(self, strokes, img_paths, index_list = None):
		...
		for i in range(len(strokes)):
			data = strokes[i]
			if self.opencv:
				img_path = '/'.join(img_paths[i].split('/')[:-1]) + '_cv/' + img_paths[i].split('/')[-1]
			else:
			...
			if len(data) <= self.max_seq_length:
				...
				if self.stroke_resize:
					min_x, max_x, min_y, max_y = get_bounds(data, 1)
					data[:, 0] *= (255 / (max_x - min_x))
					data[:, 1] *= (255 / (max_y - min_y))
	...
	def random_sample(self):
		...
		print(f'{self.png_paths[rand_idx]}')
	...
	def _get_batch_from_indices(self, indices):
		...
		for idx in range(len(indices)):
			i = indices[idx]
			strokes = np.copy(self.strokes[i])
