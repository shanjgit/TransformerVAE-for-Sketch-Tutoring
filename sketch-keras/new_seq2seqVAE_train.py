...
from new_seq2seqVAE import Seq2seqModel, get_default_hparams
...
tf.config.experimental.set_virtual_device_configuration(gpus[0], [tf.config.experimental.VirtualDeviceConfiguration(memory_limit = 1102)])
...
def get_callbacks_dict(seq2seq, model_params, experiment_path=''):
	...
	callbacks_dict['model_checkpoint'] = ModelCheckpoint(filepath=os.path.join(experiment_path, 'checkpoints',
			'weights.{epoch:02d}-{val_loss:.2f}'), monitor='val_loss', save_best_only=True, mode='min', save_weights_only=True)
	#callbacks_dict['model_checkpoint'] = ModelCheckpoint(filepath='model.h5', monitor='val_loss',
...
def main(args, hparams):
	...
	hparams_dot.index_list = False if args.index_list == None else True
	hparams_dot.opencv = args.opencv
	hparams_dot.stroke_resize = args.stroke_resize
	hparams_dot.num_layers = args.num_layers
	hparams_dot.num_heads = args.num_heads
	datasets = load_dataset(args.data_dir, hparams_dot, index_list = index_list)
...
if __name__ == '__main__':
	...
	parser = argparse.ArgumentParser(description='Main script for running sketch_pix2seq_keras')
	...
	parser.add_argument('--experiment_dir', type=str,
						default='tmp/sketch_rnn/experiments',
	...
	parser.add_argument('--opencv', action = 'store_false', help='Whether to use opencv to resize the image. (default: %(default)s)')
	
	parser.add_argument('--stroke_resize', action = 'store_false', help='Whether to resize the stroke to 255 x 255. (default: %(default)s)')
	
	parser.add_argument('--index_list', '-il', type=str, default=None, help="numpy list of desired indexs. (default: %(default)s)")
	
	parser.add_argument('--num_layers', '-nl', type=int, default=4, help="number of decoder layers. (default: %(default)s)")
	
	parser.add_argument('--num_heads', '-nh', type=int, default=8, help="number of attention heads. (default: %(default)s)")
